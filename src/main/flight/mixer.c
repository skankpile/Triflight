...
void mixTable(void)
{
    ...
    if (currentMixerMode == MIXER_TRI)
    {
        ...
        const float futureServoAngle_D = constrainf(servoAngle_D + angleDiff_D, (TRI_TAIL_SERVO_ANGLE_MID_DD - tailServoMaxAngle_DD) / 10.0f, (TRI_TAIL_SERVO_ANGLE_MID_DD + tailServoMaxAngle_DD) / 10.0f);
        currentMixer[0].throttle = 1.0f + (getPitchCorrectionAtTailAngle(futureServoAngle_D) - 1.0f) * 0.60f;
    }
