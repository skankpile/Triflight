...
STATIC_UNIT_TESTED void servoMixer(void)
{
    ...
    if (currentMixerMode == MIXER_TRI)
    {
        const float min = servoConf[SERVO_RUDDER].min;
        const float max = servoConf[SERVO_RUDDER].max;
        const float mid = servoConf[SERVO_RUDDER].middle;
        const float input = axisPID[YAW];
        const float servoRange = (input < 0.0f) ? mid - min : max - mid;
        servo[SERVO_RUDDER] = (int16_t)(mid + (input / 1000.0f * servoRange));
