...
static void resetRxSignalTimeoutIfNeeded(uint32_t currentTime)
{
    if (!rxSignalReceived) {
        return;
    }
...
{
    resetRxSignalTimeoutIfNeeded(currentTime);
...
static void detectAndApplySignalLossBehaviour(void)
{
    ...
    for (channel = 0; channel < rxRuntimeConfig.channelCount; channel++) {
        uint16_t sample = rcData[channel];

        if (isRxDataDriven() && !rxSignalReceived) {
